...
DOMAIN_LOOKUP_SOURCES = [
    ("VirusTotal", "https://www.virustotal.com/api/v3/domains/{domain} (API key required)"),
    ("WHOIS", "Standard WHOIS protocol (via python-whois)"),
]

active_domain_window = None


def show_domain_lookup_sources_dialog(parent):
    """Open a dialog listing the APIs used by Domain Lookup."""
    dlg = QDialog(parent)
    dlg.setWindowTitle("Sources — Domain Lookup")
    ...
    lines = [f"• {name}\n  {url}" for name, url in DOMAIN_LOOKUP_SOURCES]
    text.setPlainText("\n\n".join(lines))
...
def open_domain_lookup_window(parent_window, db_path):
    ...
    sources_button.clicked.connect(
        lambda: show_domain_lookup_sources_dialog(domain_window)
    )
